...
Business data path is PostgreSQL → module services → `/api` → React. The frontend may keep session tokens in `sessionStorage`, React query state, and form drafts. It must not keep an authoritative availability/trips/catalog store. Collection GETs return `{ items, page, pageSize, total }` where lists can grow. Retreat search filters (`need`, `state`, `locality`, `duration`) are applied in Catalog, including comma-separated values. Homepage sections and customer navigation come from `content.sections` and `content.navigation_*` (`GET /api/content/homepage`, `GET /api/content/navigation`).
